...
test.describe('ThermaSite mocked acceptance journey', () => {
  test('launches, ranks, rescales, cites, and reopens the preset screening', async ({ page }) => {
    ...
    await expect(page.locator('.map-caption')).toContainText('USGS aerial context')
    ...
    await expect(page.locator('.leaflet-control-attribution')).toContainText('USGS The National Map')
